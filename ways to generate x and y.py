...
# technique 4
# x = np.random.normal(mean, std, size), normally distributed numbers. not between two numbers
# generates a NumPy array of 10 random floating-point numbers drawn from a Normal (Gaussian) distribution.
# mean: Can be any float or integer. Does not have to be 0. also creates negative values.
# great for testing models or simulating moise
d = np.random.normal(1, 1, 10)
print(d)

# technique 5
# x = np.random.unifrom(low, high, size), random floating-point numbers between two numbers
# # great for creating synthetic continous data. always positive values
e = np.random.uniform(1, 100, 10)
print(e)
...
# When you create c = range(1, 10), you are creating a Python range object, not a list of numbers.
